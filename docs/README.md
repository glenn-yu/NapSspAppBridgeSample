...
## 빠른 시작 (Quick Start)
1. **환경 확인**: Android Studio(Gradle) 및 Xcode 환경을 준비합니다.
...
- iOS: Xcode 15.3 이상 권장, 현재 샘플은 `ios/Vendor/` 아래 로컬 xcframework를 포함한 상태로 빌드됨
...
- **SPM / Gradle 의존성 오류**: `docs/mediation.md`와 플랫폼별 공식 가이드를 함께 확인하세요. iOS 샘플은 현재 로컬 vendored xcframework 기준으로 검증되었습니다.
